...
mod panel_route_policy_support;
mod panel_route_support;
use panel_route_policy_support::{configured_748, paired_rank_and_bounded_gates, resume_gates};
...
#[test]
fn guided_complete_finite_domain_counts_objective_support_and_resume() {
    route_fixture(env!("CARGO_BIN_EXE_impg"), false);
}

#[test]
#[ignore = "explicit Linux frozen-f7 gate; requires IMPG_TEST_F7_FINITE"]
fn guided_frozen_f7_finite_oracle() {
    route_fixture(&panel_route_support::configured_f7(), false);
}

#[test]
#[ignore = "explicit frozen-748 backend reuse gate; requires IMPG_TEST_748_ROUTES and IMPG_TEST_POLICY_OUTPUT"]
fn guided_frozen_748_backend_reuse() {
    route_fixture(env!("CARGO_BIN_EXE_impg"), true);
}

fn route_fixture(f7: &str, parent_driver: bool) {
    let t = tempfile::tempdir().unwrap();
    let preserved = std::env::var_os("IMPG_TEST_POLICY_OUTPUT").map(std::path::PathBuf::from);
    if parent_driver {
        assert!(
            preserved.is_some(),
            "requested frozen-748 gate requires IMPG_TEST_POLICY_OUTPUT"
        );
    }
    ...
    .unwrap();
    // Differential decoding through the evaluator's verified records and shared seeks.
    let mut clone = evaluator.ports.global.try_clone().unwrap();
    for index in 0..graph.port_count {
        let decoded = policy_adapter::read(&mut clone, graph.k, graph.port_count, index).unwrap();
        policy_adapter::verified(&decoded, &graph).unwrap();
        let backend = evaluator
            .ports
            .at_cut(
                &graph,
                decoded.source,
                decoded.cut(graph.k).unwrap(),
                decoded.reverse,
            )
            .unwrap()
            .unwrap();
        assert_eq!(decoded.word, backend.word);
        assert_eq!(
            (decoded.source, decoded.anchor, decoded.reverse),
            (backend.source, backend.anchor, backend.reverse)
        );
        if index % 11 == 0 {
            evaluator
                .evaluate(&graph.native_assignment(0).unwrap())
                .unwrap();
        }
        assert_eq!(
            policy_adapter::read(&mut clone, graph.k, graph.port_count, index).unwrap(),
            decoded
        );
    }
    let mut invalid_port = policy_adapter::read(&mut clone, graph.k, graph.port_count, 0).unwrap();
    invalid_port.source = graph.lanes.len();
    assert!(policy_adapter::verified(&invalid_port, &graph).is_err());
    invalid_port.source = 0;
    invalid_port.anchor = u64::MAX;
    assert!(policy_adapter::verified(&invalid_port, &graph).is_err());
    ...
    assert_eq!(r["evaluations"].as_u64().unwrap(), expected.len() as u64);
    assert!(r["native_score_reuses"].as_u64().unwrap() > 0);
    ...
    resume_gates(root, &prefix, &graph_dir, &sample_path, &searched);
    paired_rank_and_bounded_gates(root, &prefix, &graph_dir, &panel, &graph, &a, &b);
    ...
    let extended_support = root.join("extended-lost-support");
    run(
        binary,
        &[
            "genome-infer",
    ...
    let sticky = read(&extended_support.join("result.json"));
    assert_eq!(sticky["result"]["sticky_support_loss"], true);
    assert_eq!(sticky["result"]["correlated_optima_complete"], false);
    assert_eq!(
        sticky["result"]["evaluations"],
        cap_result["result"]["evaluations"]
    );
