...
fn coupling_fixture(f7: &str) {
    ...
    let preserved =
        std::env::var_os("IMPG_TEST_POLICY_COUPLING_OUTPUT").map(std::path::PathBuf::from);
    if let Some(path) = &preserved {
        assert!(path.is_absolute());
        fs::create_dir(path).expect("fresh retained coupling fixture");
    }
    ...
    assert!(r["result"]["completion_probe_conflicts"].as_u64().unwrap() > 0);
    assert!(r["result"]["completion_probes_completed"].as_u64().unwrap() > 0);
    let mut prior: Option<std::path::PathBuf> = None;
    let mut phases = BTreeSet::new();
    for work in (1..=20).chain([32, 64, 128, 256, 512]) {
        let output = root.join(format!("probe-resume-{work}"));
        ...
        if let Some(parent) = &prior {
            args.extend(["--resume-from", p(parent), "--extend-budgets"]);
        }
        run(bin, &args);
        for task in read(&output.join("checkpoint.json"))["state"]["tasks"]
            .as_object()
            .unwrap()
            .values()
        {
            if let Some(op) = task["op"].as_object() {
                phases.extend(op.keys().cloned());
            }
        }
        assert!(fs::read(searched.join("evaluations.jsonl"))
            .unwrap()
            .starts_with(&fs::read(output.join("evaluations.jsonl")).unwrap()));
        prior = Some(output);
    }
    assert!(
        phases.contains("Probe") && phases.contains("ProbeCheck"),
        "missing probe resume boundaries: {phases:?}"
    );
    let resumed = root.join("probe-resumed-complete");
    run(
        bin,
        &[
            "genome-infer",
            ...
            p(&resumed),
            "--resume-from",
            p(prior.as_ref().unwrap()),
            "--extend-budgets",
        ],
    );
    assert_eq!(
        fs::read(resumed.join("evaluations.jsonl")).unwrap(),
        fs::read(searched.join("evaluations.jsonl")).unwrap()
    );
    assert_eq!(
        read(&resumed.join("checkpoint.json"))["state"],
        read(&searched.join("checkpoint.json"))["state"]
    );
