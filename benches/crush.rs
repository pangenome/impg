...
const DEFAULT_BENCH_GFA: &str =
    "data/c4_crush_eval_20260523T140141Z/C4A.blunt.gfa";
...
fn bench_crush(c: &mut Criterion) {
    ...
    eprintln!(
        "crush bench shape: {} segments, {} paths",
        n_seg, n_path
    );
    ...
    group.bench_function(BenchmarkId::new("parse_gfa", "C4A.blunt"), |b| {
        b.iter(|| {
            impg::resolution::bench_parse_gfa(&gfa_text)
                .expect("parse must succeed in bench")
        })
    });

    group.bench_function(BenchmarkId::new("parse_render_roundtrip", "C4A.blunt"), |b| {
        b.iter(|| {
    ...
    group.bench_function(BenchmarkId::new("validate_streaming_self", "C4A.blunt"), |b| {
        b.iter(|| {
