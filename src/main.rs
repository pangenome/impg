...
const READ_WALK_INDEX_VERSION: u32 = 1;
...
struct ReadWalkIndexStats {
    ...
    gbwt_bytes: u64,
    loc_bytes: u64,
    loc_sample_bytes: u64,
    node_bytes: u64,
    read_walk_nodes: u64,
}

impl ReadWalkIndexStats {
    fn add(&mut self, other: Self) {
        self.reads += other.reads;
        self.retained_reads += other.retained_reads;
        self.matched_syncmers += other.matched_syncmers;
        self.path_steps += other.path_steps;
        self.locator_samples += other.locator_samples;
        self.chunks += other.chunks;
        self.gbwt_bytes += other.gbwt_bytes;
        self.loc_bytes += other.loc_bytes;
        self.loc_sample_bytes += other.loc_sample_bytes;
        self.node_bytes += other.node_bytes;
        self.read_walk_nodes += other.read_walk_nodes;
    }
}
...
struct ReadWalkPathChunk {
    paths: Vec<ReadWalkPath>,
    stats: ReadWalkIndexStats,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct ReadWalkLocatorSample {
    signed_node: i32,
    rank: u32,
    read_ordinal: u64,
    step_idx: u32,
...
#[derive(Default)]
struct ReadWalkNodeDictionary {
    syng_to_internal: FxHashMap<u32, i32>,
    internal_to_syng: Vec<u32>,
}

impl ReadWalkNodeDictionary {
    fn intern_signed_node(&mut self, signed_node: i32) -> i32 {
        let syng_node = signed_node.unsigned_abs();
        let internal = if let Some(&internal) = self.syng_to_internal.get(&syng_node) {
            internal
        } else {
            let next_id = (self.internal_to_syng.len() + 1) as i32;
            self.internal_to_syng.push(syng_node);
            self.syng_to_internal.insert(syng_node, next_id);
            next_id
        };
        if signed_node < 0 {
            -internal
    ...
    fn remap_path(&mut self, path: &mut ReadWalkPath) {
        for (signed_node, _) in &mut path.steps {
            *signed_node = self.intern_signed_node(*signed_node);
        }
    }

    fn len(&self) -> usize {
        self.internal_to_syng.len()
    }
}
...
fn read_walk_index_base(prefix: &str) -> &str {
    prefix.strip_suffix(".rwalk").unwrap_or(prefix)
}

fn read_walk_index_path(prefix: &str, extension: &str) -> String {
    format!("{}.rwalk.{}", read_walk_index_base(prefix), extension)
}
...
struct OwnedSyngBwt {
    ptr: *mut impg::syng_ffi::SyngBWT,
}

unsafe impl Send for OwnedSyngBwt {}

impl OwnedSyngBwt {
    fn new(params: impg::syng::SyncmerParams) -> io::Result<Self> {
        let syncmer_len = params.k.checked_add(params.w).ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "syncmer length overflowed")
        })?;
        let ptr = unsafe { impg::syng_ffi::syngBWTcreate(syncmer_len as i32, 0) };
        if ptr.is_null() {
    ...
    fn as_mut_ptr(&mut self) -> *mut impg::syng_ffi::SyngBWT {
        self.ptr
    }
}

impl Drop for OwnedSyngBwt {
    fn drop(&mut self) {
        unsafe {
            if !self.ptr.is_null() {
                impg::syng_ffi::syngBWTdestroy(self.ptr);
            }
        }
    }
}

fn sample_read_walk_step(step_idx: u32, last_step_idx: u32, locator_step_rate: u32) -> bool {
    step_idx == last_step_idx || (last_step_idx - step_idx) % locator_step_rate == 0
}

fn build_read_walk_path_chunk(
    syng_matcher: &mut impg::syng::SyngMatcherWorker<'_>,
    base_read_ordinal: u64,
    chunk: &SyngGafQueryChunk,
    min_syncmers: usize,
) -> io::Result<ReadWalkPathChunk> {
    let mut stats = ReadWalkIndexStats {
        reads: chunk.len() as u64,
        chunks: 1,
        ..ReadWalkIndexStats::default()
    };
    ...
    for (read_offset, record) in chunk.records.iter().enumerate() {
        ...
        syncmers.dedup_by(|a, b| a.query_pos == b.query_pos && a.signed_node == b.signed_node);
        if syncmers.len() < min_syncmers {
            continue;
        }

        let mut steps = Vec::with_capacity(syncmers.len());
        for syncmer in &syncmers {
            let query_pos = u32::try_from(syncmer.query_pos).map_err(|_| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "read syncmer position exceeds u32",
                )
            })?;
            steps.push((syncmer.signed_node, query_pos));
        }
        stats.retained_reads += 1;
        stats.path_steps += steps.len() as u64;
        paths.push(ReadWalkPath {
            read_ordinal: base_read_ordinal + read_offset as u64,
            steps,
        });
    }

    Ok(ReadWalkPathChunk { paths, stats })
}

fn add_read_walk_path_to_gbwt(
    gbwt: *mut impg::syng_ffi::SyngBWT,
    path: &ReadWalkPath,
    locator_step_rate: u32,
    locator_samples: &mut Vec<ReadWalkLocatorSample>,
) -> io::Result<u64> {
    if path.steps.is_empty() {
        return Ok(0);
    }
    let last_step_idx = (path.steps.len() - 1) as u32;
    let mut samples_added = 0u64;
    unsafe {
        let (first_node, first_pos) = path.steps[0];
        let sbp = impg::syng_ffi::syngBWTpathStartNew(gbwt, first_node);
        if sbp.is_null() {
            return Err(io::Error::other("syngBWTpathStartNew returned null"));
        }
        if sample_read_walk_step(0, last_step_idx, locator_step_rate) {
            locator_samples.push(ReadWalkLocatorSample {
                signed_node: first_node,
                rank: (*sbp).j_last,
                read_ordinal: path.read_ordinal,
                step_idx: 0,
            });
            samples_added += 1;
        }
        ...
        for (step_idx, &(next_node, next_pos)) in path.steps.iter().enumerate().skip(1) {
            let offset = next_pos.checked_sub(previous_pos).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "read-walk syncmer positions are not sorted",
                )
            })?;
            impg::syng_ffi::syngBWTpathAdd(sbp, next_node, offset);
            let step_idx = step_idx as u32;
            if sample_read_walk_step(step_idx, last_step_idx, locator_step_rate) {
                locator_samples.push(ReadWalkLocatorSample {
        ...
        impg::syng_ffi::syngBWTpathFinish(sbp);
    }
    Ok(samples_added)
}

fn write_read_walk_gbwt(gbwt: &mut OwnedSyngBwt, path: &str) -> io::Result<u64> {
    let cpath =
        CString::new(path).map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e))?;
    let gbwt_type = CString::new("gbwt").unwrap();
    let schema_text = impg::syng_ffi::syng_schema_text();
    unsafe {
        let schema = impg::syng_ffi::oneSchemaCreateFromText(schema_text.as_ptr());
        if schema.is_null() {
            return Err(io::Error::other("failed to create ONEcode schema for read-walk GBWT"));
        }
        let of = impg::syng_ffi::oneFileOpenWriteNew(
            cpath.as_ptr(),
            schema,
            gbwt_type.as_ptr(),
            true,
        ...
        if of.is_null() {
            impg::syng_ffi::oneSchemaDestroy(schema);
            return Err(io::Error::other(format!(
                "failed to open {} for writing",
                path
            )));
        }
        impg::syng_ffi::syngBWTwrite(of, gbwt.as_mut_ptr());
        impg::syng_ffi::oneFileClose(of);
        impg::syng_ffi::oneSchemaDestroy(schema);
    }
    Ok(std::fs::metadata(path)?.len())
}

fn write_read_walk_locator_sample_entry<W: Write>(
    out: &mut W,
    signed_node: i32,
    previous_signed_node: i32,
    offset: u64,
) -> io::Result<()> {
    out.write_all(&signed_node.to_le_bytes())?;
    out.write_all(&previous_signed_node.to_le_bytes())?;
    out.write_all(&offset.to_le_bytes())?;
    Ok(())
}

fn write_read_walk_locator_files(
    output_prefix: &str,
    locator_samples: &mut Vec<ReadWalkLocatorSample>,
    locator_node_sample_rate: u32,
    stats: &mut ReadWalkIndexStats,
) -> io::Result<()> {
    locator_samples.par_sort_unstable_by(|a, b| {
        a.signed_node
            .cmp(&b.signed_node)
            .then(a.rank.cmp(&b.rank))
            .then(a.read_ordinal.cmp(&b.read_ordinal))
            .then(a.step_idx.cmp(&b.step_idx))
    });
    locator_samples.dedup();

    let loc_path = read_walk_index_path(output_prefix, "loc");
    let loc_sample_path = read_walk_index_path(output_prefix, "lsample");
    let mut loc_out = BufWriter::new(File::create(&loc_path)?);
    let mut sample_out = BufWriter::new(File::create(&loc_sample_path)?);
    loc_out.write_all(b"IMPG-RWLOC\x01")?;
    let mut loc_offset = 11u64;
    let mut i = 0usize;
    let mut observed_nodes = 0u64;
    let mut previous_signed_node = 0i32;
    stats.locator_samples = locator_samples.len() as u64;

    while i < locator_samples.len() {
        let signed_node = locator_samples[i].signed_node;
        if observed_nodes % u64::from(locator_node_sample_rate) == 0 {
            write_read_walk_locator_sample_entry(
                &mut sample_out,
                signed_node,
                previous_signed_node,
                loc_offset,
            )?;
            stats.loc_sample_bytes += 16;
        }

        let group_start = i;
        while i < locator_samples.len() && locator_samples[i].signed_node == signed_node {
            i += 1;
        }
        loc_offset += write_varint(&mut loc_out, encode_i32_varint(signed_node))?;
        loc_offset += write_varint(&mut loc_out, (i - group_start) as u64)?;

        let mut previous_rank = 0u32;
        let mut first_rank = true;
        for sample in &locator_samples[group_start..i] {
            let rank_delta = if first_rank {
                sample.rank
            } else {
                sample.rank.checked_sub(previous_rank).ok_or_else(|| {
                    io::Error::new(io::ErrorKind::InvalidData, "locator ranks are not sorted")
                })?
            };
            loc_offset += write_varint(&mut loc_out, u64::from(rank_delta))?;
            loc_offset += write_varint(&mut loc_out, sample.read_ordinal)?;
            loc_offset += write_varint(&mut loc_out, u64::from(sample.step_idx))?;
            previous_rank = sample.rank;
            first_rank = false;
        }
        previous_signed_node = signed_node;
        observed_nodes += 1;
    }

    loc_out.flush()?;
    sample_out.flush()?;
    stats.loc_bytes = loc_offset;
    Ok(())
}

fn write_read_walk_nodes_file(
    output_prefix: &str,
    dictionary: &ReadWalkNodeDictionary,
) -> io::Result<u64> {
    let nodes_path = read_walk_index_path(output_prefix, "nodes");
    let mut out = BufWriter::new(File::create(&nodes_path)?);
    out.write_all(b"IMPG-RWNOD\x01")?;
    let mut bytes = 11u64;
    bytes += write_varint(&mut out, dictionary.internal_to_syng.len() as u64)?;
    for &syng_node in &dictionary.internal_to_syng {
        bytes += write_varint(&mut out, u64::from(syng_node))?;
    }
    out.flush()?;
    Ok(bytes)
}

fn stream_read_walk_index_query_paths(
    paths: &[String],
    tx: &channel::Sender<(u64, SyngGafQueryChunk)>,
) -> io::Result<()> {
    let mut next_read_ordinal = 0u64;
    for path in paths {
        info!("Streaming read-walk-index query file: {}", path);
        stream_query_chunks_with(path, |_, chunk| {
            let base_read_ordinal = next_read_ordinal;
            next_read_ordinal = next_read_ordinal
                .checked_add(chunk.len() as u64)
                .ok_or_else(|| io::Error::other("read ordinal overflow"))?;
            tx.send((base_read_ordinal, chunk)).map_err(|_| {
                io::Error::new(
                    io::ErrorKind::BrokenPipe,
                    "failed to send query chunk to read-walk-index workers",
                )
            })
        })?;
    }
...
fn build_read_walk_index(
    ...
    if locator_step_rate == 0 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "--locator-step-rate must be greater than 0",
        ));
    }
    ...
    let queue_capacity = threads.saturating_mul(4).max(1);
    let (query_tx, query_rx) = channel::bounded::<(u64, SyngGafQueryChunk)>(queue_capacity);
    let (path_tx, path_rx) = channel::bounded::<ReadWalkPathChunk>(queue_capacity);

    let (mut gbwt, dictionary, mut locator_samples, mut stats) = std::thread::scope(|scope| {
        let collector = scope.spawn(move || -> io::Result<(
            OwnedSyngBwt,
            ReadWalkNodeDictionary,
            Vec<ReadWalkLocatorSample>,
            ReadWalkIndexStats,
        )> {
            let mut gbwt = OwnedSyngBwt::new(syng_matcher.params)?;
            let mut dictionary = ReadWalkNodeDictionary::default();
            let mut locator_samples = Vec::new();
            let mut stats = ReadWalkIndexStats::default();
            let mut next_log_reads = 10_000_000u64;
            while let Ok(chunk) = path_rx.recv() {
                stats.add(chunk.stats);
                for mut path in chunk.paths {
                    dictionary.remap_path(&mut path);
                    stats.locator_samples += add_read_walk_path_to_gbwt(
                        gbwt.as_mut_ptr(),
                        &path,
                        locator_step_rate,
                        &mut locator_samples,
                ...
                if stats.reads >= next_log_reads {
                    info!(
                        "Read-walk-index scanned {} reads, retained {}, path_steps {}, read_walk_nodes {}, locator_samples {}",
                        stats.reads,
                        stats.retained_reads,
                        stats.path_steps,
                        dictionary.len(),
                        stats.locator_samples
                    );
                    while next_log_reads <= stats.reads {
                        next_log_reads += 10_000_000;
                    }
            ...
            stats.read_walk_nodes = dictionary.len() as u64;
            Ok((gbwt, dictionary, locator_samples, stats))
        });
        ...
        for _ in 0..threads {
            ...
            workers.push(scope.spawn(move || -> io::Result<()> {
                let mut worker = syng_matcher.worker().map_err(|e| {
                    io::Error::new(
                        e.kind(),
                        format!("failed to create read-walk-index syng worker: {e}"),
                    )
                })?;
                while let Ok((base_read_ordinal, chunk)) = query_rx.recv() {
                    let path_chunk = build_read_walk_path_chunk(
                        &mut worker,
                        base_read_ordinal,
                        &chunk,
                        min_syncmers,
                    )?;
                    path_tx.send(path_chunk).map_err(|_| {
                        io::Error::new(
                            io::ErrorKind::BrokenPipe,
                            "failed to send read-walk paths to collector",
                        )
                    })?;
                }
                Ok(())
        ...
        let reader_result = stream_read_walk_index_query_paths(query_paths, &query_tx);
        drop(query_tx);
        ...
        for worker in workers {
            match worker.join() {
                Ok(Ok(())) => {}
                Ok(Err(e)) => worker_errors.push(e.to_string()),
                Err(_) => worker_errors.push("read-walk-index worker thread panicked".to_string()),
            }
        }

        let collector_result = collector
            .join()
            .map_err(|_| io::Error::other("read-walk-index collector thread panicked"))?;
        reader_result?;
        if !worker_errors.is_empty() {
            return Err(io::Error::other(worker_errors.join("; ")));
        }
    ...
    info!(
        "Sorting {} read-walk locator samples",
        locator_samples.len()
    );
    let sort_start = Instant::now();
    let locator_node_sample_rate = 256u32;
    write_read_walk_locator_files(
        output_prefix,
        &mut locator_samples,
        locator_node_sample_rate,
        &mut stats,
    )?;
    info!(
        "Sorted and wrote read-walk locator in {:.3}s",
        sort_start.elapsed().as_secs_f64()
    );

    let gbwt_path = read_walk_index_path(output_prefix, "1gbwt");
    let gbwt_write_start = Instant::now();
    stats.gbwt_bytes = write_read_walk_gbwt(&mut gbwt, &gbwt_path)?;
    info!(
        "Wrote read-walk GBWT in {:.3}s",
        gbwt_write_start.elapsed().as_secs_f64()
    );

    stats.read_walk_nodes = dictionary.len() as u64;
    stats.node_bytes = write_read_walk_nodes_file(output_prefix, &dictionary)?;

    let meta_path = read_walk_index_path(output_prefix, "meta");
    let loc_path = read_walk_index_path(output_prefix, "loc");
    let loc_sample_path = read_walk_index_path(output_prefix, "lsample");
    let nodes_path = read_walk_index_path(output_prefix, "nodes");
    let meta = serde_json::json!({
        "format": "impg-rwalk",
        "version": READ_WALK_INDEX_VERSION,
        "description": "orientation-correct read syncmer walks over a dense subset of syng node ids, plus sampled read locator",
        "syng_prefix": syng_prefix,
        "query_files": query_paths,
        "syncmer": {
            "smer_length": syng_matcher.params.k,
            "syncmer_length": syng_matcher.params.k + syng_matcher.params.w,
            "seed": syng_matcher.params.seed,
        },
        "min_distinct_syncmers_per_read": min_syncmers,
        "locator_step_rate": locator_step_rate,
        "locator_node_sample_rate": locator_node_sample_rate,
        "reads": stats.reads,
        "retained_reads": stats.retained_reads,
        "matched_syncmers": stats.matched_syncmers,
        "path_steps": stats.path_steps,
        "read_walk_nodes": stats.read_walk_nodes,
        "locator_samples": stats.locator_samples,
        "gbwt_bytes": stats.gbwt_bytes,
        "node_bytes": stats.node_bytes,
        "loc_bytes": stats.loc_bytes,
        "loc_sample_bytes": stats.loc_sample_bytes,
        "build_seconds": start.elapsed().as_secs_f64(),
        "files": {
            "gbwt": gbwt_path,
            "nodes": nodes_path,
            "loc": loc_path,
            "lsample": loc_sample_path,
        }
    });
    let mut meta_out = BufWriter::new(File::create(&meta_path)?);
    serde_json::to_writer_pretty(&mut meta_out, &meta).map_err(io::Error::other)?;
    ...
    info!(
        "Built read-walk-index for {} reads: retained {}, path_steps {}, read_walk_nodes {}, locator_samples {}, gbwt_bytes {}, node_bytes {}, loc_bytes {} in {:.3}s",
        stats.reads,
        stats.retained_reads,
        stats.path_steps,
        stats.read_walk_nodes,
        stats.locator_samples,
        stats.gbwt_bytes,
        stats.node_bytes,
        stats.loc_bytes,
...
enum Args {
    ...
    /// Build a GBWT over orientation-correct read syncmer walks
    ReadWalkIndex {
        /// Syng index prefix or .1khash/.meta path
        #[clap(short = 'a', long, value_parser)]
        index: String,
        ...
        /// Output prefix (produces .rwalk.1gbwt, .rwalk.nodes, .rwalk.loc, .rwalk.lsample, .rwalk.meta)
        #[clap(short = 'o', long, value_parser)]
        output: String,
        ...
        /// Sample every N read syncmer steps from the terminal side for read lookup
        #[clap(long, value_parser, default_value_t = 256)]
        locator_step_rate: u32,
...
mod tests {
    ...
    fn read_test_i32_le(bytes: &[u8], offset: &mut usize) -> i32 {
        let end = *offset + 4;
    ...
    #[test]
    fn test_read_walk_index_path_avoids_double_suffix() {
        assert_eq!(
            read_walk_index_path("sample", "loc"),
            "sample.rwalk.loc"
        );
        assert_eq!(
            read_walk_index_path("sample.rwalk", "loc"),
            "sample.rwalk.loc"
        );
    }

    #[test]
    fn test_read_walk_sampling_from_terminal_side() {
        assert!(sample_read_walk_step(9, 9, 4));
        assert!(sample_read_walk_step(5, 9, 4));
        assert!(sample_read_walk_step(1, 9, 4));
        assert!(!sample_read_walk_step(0, 9, 4));
        assert!(!sample_read_walk_step(4, 9, 4));
    }
    ...
    #[test]
    fn test_write_read_walk_locator_files_encodes_samples() {
        let dir = tempfile::tempdir().unwrap();
        let output = dir.path().join("tiny.rwalk");
        let output = output.to_string_lossy().to_string();
        let mut samples = vec![
            ReadWalkLocatorSample {
                signed_node: -7,
                rank: 3,
                read_ordinal: 9,
                step_idx: 2,
        ...
        let mut stats = ReadWalkIndexStats::default();
        write_read_walk_locator_files(&output, &mut samples, 1, &mut stats).unwrap();
        assert_eq!(stats.locator_samples, 3);
        assert_eq!(stats.loc_sample_bytes, 32);

        let lsample = std::fs::read(read_walk_index_path(&output, "lsample")).unwrap();
        assert_eq!(lsample.len(), 32);
        let mut offset = 0usize;
        assert_eq!(read_test_i32_le(&lsample, &mut offset), -7);
        assert_eq!(read_test_i32_le(&lsample, &mut offset), 0);
        assert_eq!(read_test_u64_le(&lsample, &mut offset), 11);
        assert_eq!(read_test_i32_le(&lsample, &mut offset), 5);
        assert_eq!(read_test_i32_le(&lsample, &mut offset), -7);
        assert!(read_test_u64_le(&lsample, &mut offset) > 11);

        let loc = std::fs::read(read_walk_index_path(&output, "loc")).unwrap();
        assert!(loc.starts_with(b"IMPG-RWLOC\x01"));
        assert_eq!(
            decode_test_varints(&loc[11..]),
            vec![
                encode_i32_varint(-7),
                1,
                3,
