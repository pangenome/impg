...
Status: **Gate A, B1, bounded B2, the finite diploid foundation and one automatic joint-diploid dosage recovery accepted in finite prototype scope; full Gate B and genome-scale recovery remain incomplete, 2026-09-15**. The observation operator and complete-genome scorer exist; reliable genome-scale mosaic recovery does not yet exist. [Gate A's executable finite diagnostic](structured-poisson-finite-diagnostic.md) was independently reviewed and parent-reproduced (commit `9d8cc24`): public counts, exhaustive feasible optima/support and numerical bounds agreed; 699 portable Rust tests and the configured gates passed. This is finite numerical evidence, not a genome-scale certificate. [B1's score-guided generator](panel-route-residual-search.md), commit `594a6f3`, now constructs new complete mosaics and changes subsequent refinement geometry in response to sample counts. Independent review and parent reruns passed, including 706 portable tests and the original resource-boundary red/green cases. It recovered the exact frozen 1,936bp two-molecule synthetic truth, improving the objective by 67.5373 over every native. This is not whole-yeast recovery. The opt-in B2 primary construction slice, commit `fcd5b9a`, passed fresh review and parent reruns (710 portable tests plus configured gates): exact560bp two-donor recovery and automatic opposite-strand single-span exchanges. Positive opposite transport of an interior seam remains open. Those B1/B2 results are haploid-oriented, not diploid evidence. The separate [diploid representation/counts/identifiability foundation](diploid-foundation.md), commit `d35b00d`, is now independently reviewed and parent-reproduced: 712 portable tests, 24 exhaustive supplied paired hypotheses, correct two-copy multiplicity/heterozygous dosage/near phase, and exactly equivalent far-phase alternatives retained. The subsequent [joint-diploid dosage search](diploid-joint-dosage-search.md) now automatically recovers a count-exact `[0,1,1,2]` dosage state in which both primary copies must be non-native mosaics. This is reviewed and parent-reproduced finite evidence, not genome-scale or production acceptance. The next implementation is specified in the [partition-local diplotype inference and physical chaining guide](partition-local-diplotype-inference.md): LikeGT-style exhaustive local allele-pair construction over the stronger MEM-BWT feature space, followed by physical phase chaining and exact global rescoring.

## Prospective information-aware successor

The user accepts information-limited phase. The [information-aware diploid
gate](diploid-information-aware-gate.md) prospectively tests observable dosage/count
recovery on the unchanged far-phase fixture plus informative automatic near-phase
native-pair recovery. This changes tests/assessment, not engine policy or budgets.
The previous exhaustive CIS/TRANS generation assertion remains an explicitly
runnable historical negative; missing TRANS and all old failures remain recorded.
Exact reference count equality witnesses a specific ambiguity, whereas incomplete
search alone proves neither information limits nor genotype uniqueness. No full
Gate B, production, global-support or sequence-emission acceptance follows.
...
### Gate B — structured complete-genome proposal/pricing experiment (IN PROGRESS; B1 and bounded B2 primary constructions accepted)

**B1 exit:** automatic single-donor replacements, score-dependent refinement and supported same-orientation exchanges passed the frozen tiny recovery/causal/physical-resource tests. Exact sparse complete-objective deltas drive feedback; the diagnostic linear residual price is not advertised as a cheaper oracle. Seven retained best physical assignments spell the exact test genome; support remains incomplete. B2 subsequently established automatic exact-cut opposite-strand single-span exchanges and return-hub-driven two-donor/no-direct-return construction; it also corrected independent-channel service and downstream-stop confirmation accounting. Positive opposite transport of an interior seam remains unproved. Frozen tiny-test resource caps cannot initialize a whole yeast genome; no genome-scale, diploid or full Gate-B acceptance follows from these results.
...
### Diploid foundation — ACCEPTED, restricted finite scope

The [fixed-diploid foundation](diploid-foundation.md) passed independent review and parent reproduction. Two whole-copy slots permit A/A from one source while preserving unchanged within-copy global capacity. Checked integer profiles combine before one shared loss. All24 supplied pairs were exhausted; the heterozygous and near-phase truth pairs are sole optima, while far coupling/repulsion have exactly equal vectors and remain correlated optima. Copy-injective whole-molecule assessment detects the missing-copy negative. The singleton A/A domain validates representation, not inferred ploidy. This is not automatic diploid generation or biological validation.

### Joint diploid construction — REVIEWED FINITE RECOVERY

The [joint-diploid dosage search](diploid-joint-dosage-search.md) adds one generic operator to the experimental paired host. Complete alternatives already generated for an immutable baseline contribute physical source segments and traversal-coordinate breakpoints. Bounded projections place those segments into the other copy, then undergo orientation-seam checks, public validation and the unchanged complete paired scorer. No triggering single-copy improvement is required.

The unchanged search failed the predeclared mosaic-required fixture at dosage `[0,1,1,1]` and loss -1645.1430224277594. The joint operator recovered two count-exact retained pairs with dosage `[0,1,1,2]`, both primary copies non-native, and loss -1679.8676690136972, equal to the restricted153-pair minimum. The two whole phases remain count-equivalent because the heterozygous tracts are beyond L150 linkage.

Independent review found and then closed one temporary-state issue: projection keys are retained only after public validation and successful scoring, bounded to the current key plus16 scored projections, with all17 keys explicitly reserved. Parent reproduction passed726 portable tests with27 ignored; the historical exhaustive far-phase support test remains an explicit expected failure. The result is small-scale and nearly consumes the existing4M work cap. Scaling, broader orientation/molecule recovery and production integration remain open.
...
Supplied exhaustive finite identifiable fixtures must recover their optimum/support.
Automatic information-aware fixtures require observable recovery and honest incomplete
support under their prospectively frozen criteria; exhaustive generation of every
count-equivalent far-phase alternative is not required. Informative linkage must
still be recovered, and a missed identifiable phase remains a search failure. Genome-scale acceptance thresholds and ambiguity handling must be declared per challenge before outcomes; current data must not be relabeled blind after tuning. Coverage loss is not repaired merely because aligned QV increases. Do not collapse score-tied physical origins into a certified winner.
