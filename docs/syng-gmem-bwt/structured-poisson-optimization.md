...
### Diploid foundation — NEXT, before further genome-scale Gate B/C work

Follow [the fixed-diploid foundation contract](diploid-foundation.md). Explicitly represent two copies, including A/A from a single panel source; sum checked per-length counts and signals before one global loss; distinguish identifiable local phase from count-equivalent long-range paired alternatives. Require finite exhaustive, reviewed and parent-reproduced evidence. Two molecules or two panel sources do not establish diploid support. This foundation is not automatic diploid generation or biological validation; stock identity/unknown ploidy and other broader claims remain later gates.
