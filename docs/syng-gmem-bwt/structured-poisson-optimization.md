...
### Joint diploid construction — NEXT, design first within Gate B

Generate new non-native paired mosaics from panel/count inputs using the complete paired objective, not independent haploid calls or a supplied answer list. Begin with a frozen small supported-topology construction and an actual-operator phase-ambiguity control. Preserve the partner's signal, per-copy resource constraints, immutable baseline/slot bindings, exploratory/compound opportunities, and paired alternatives. Reuse existing geometry without a wholesale search fork or silent B1/B2 changes. Specify count-responsive construction, caps, exact score accounting, finite oracle checks and copy-aware assessment before edits/outcomes. Separate supplied oracle alternatives from inference inputs. Success remains small-scale; practical genome-scale resources and a fresh blind recovery challenge remain required.
